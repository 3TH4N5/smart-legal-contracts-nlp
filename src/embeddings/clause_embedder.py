...
class LegalBERTClauseEmbedder:
    ...
    def load_legalbert_models(self):
        ...
        try:
            ...
            logger.info(
                f"LegalBERT multiclass classifier loaded ({len(self.clause_types)} clause types)"
            )
    ...
    def predict_with_legalbert(
        ...
        logger.info(
            f"Binary classification: {len(positive_indices)}/{len(texts)} positive samples"
        )
        ...
        logger.info(
            f"LegalBERT classification complete: {len(multiclass_predictions)} classified clauses"
        )

        return binary_predictions, multiclass_predictions, positive_indices

    def extract_classified_clauses(
        self, data_splits: Dict[str, List[Dict]]
    ) -> List[Dict]:
        ...
        for split_name, data_list in data_splits.items():
            ...
            binary_preds, multiclass_preds, positive_indices = (
                self.predict_with_legalbert(texts)
            )
                ...
                if binary_preds[i] == 1:  # Positive sample
                    ...
                    if multiclass_idx < len(multiclass_preds):
                        ...
                        predicted_clause_type = (
                            self.clause_types[class_idx]
                            if class_idx < len(self.clause_types)
                            else "unknown"
                        )
                        multiclass_idx += 1
                    else:
        ...
        logger.info(
            f"Extracted {len(all_classified_clauses)} high-quality clauses using LegalBERT"
        )
        return all_classified_clauses

    def generate_embeddings(
        self, clauses: List[Dict], text_field: str = "answer_text"
    ) -> np.ndarray:
        """
        ...
        """
        logger.info(
            f"Generating embeddings for {len(clauses)} LegalBERT-classified clauses..."
        )
        ...
        for clause in clauses:
            ...
            if not text or text.strip() == "" or text.strip() == "nan":
                # Fallback hierarchy
                text = clause.get("qa_combination", "") or clause.get(
                    "question", "No text available"
                )
            texts.append(text)
    ...
    def create_embedding_index(self, clauses: List[Dict], embeddings: np.ndarray):
        ...
        self.embedding_index = {
            "clause_id_to_index": {
                clause["clause_id"]: i for i, clause in enumerate(clauses)
            },
            "predicted_clause_type_to_indices": self._group_by_predicted_clause_type(
                clauses
            ),
            "ground_truth_clause_type_to_indices": self._group_by_ground_truth_clause_type(
                clauses
            ),
            "split_to_indices": self._group_by_split(clauses),
            "legalbert_metadata": {
                "total_classified": len(clauses),
                "unique_predicted_types": len(
                    set(c["predicted_clause_type"] for c in clauses)
                ),
    ...
    def _group_by_predicted_clause_type(
    ...
    def _group_by_ground_truth_clause_type(
        self, clauses: List[Dict]
    ) -> Dict[str, List[int]]:
    ...
    def save_embeddings(self, filename: str = None):
        ...
        if filename is None:
            filename = (
                f"legalbert_clause_embeddings_{self.model_name.replace('/', '_')}"
            )
    ...
    def get_embedding_stats(self) -> Dict:
        ...
        stats = {
            ...
            "ground_truth_clause_type_distribution": {
                clause_type: len(indices)
                for clause_type, indices in self.embedding_index[
                    "ground_truth_clause_type_to_indices"
                ].items()
            },
            "split_distribution": {
                split: len(indices)
            ...
            "legalbert_pipeline_info": self.embedding_index.get(
                "legalbert_metadata", {}
            ),
        }
...
def main():
    ...
    embeddings = embedder.generate_embeddings(
        classified_clauses, text_field="answer_text"
    )
    ...
    predicted_types = sorted(
        stats["predicted_clause_type_distribution"].items(),
        key=lambda x: x[1],
        reverse=True,
    )
