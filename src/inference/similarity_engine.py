...
class LegalBERTSimilarityEngine:
    ...
    def load_legalbert_models(self):
        ...
        try:
            ...
            self.multiclass_tokenizer = AutoTokenizer.from_pretrained(
    ...
    def load_embeddings(self, filename: str):
        ...
        if extraction_method == "LegalBERT_Pipeline":
            ...
            logger.warning(
                "Using baseline embeddings - consider regenerating with LegalBERT"
            )
    ...
    def classify_query_with_legalbert(
        self, query_text: str, threshold: float = 0.3
        ...
        with torch.no_grad():
            ...
            if has_answer:
                ...
                confidence = (
                    torch.nn.functional.softmax(outputs.logits, dim=-1).max().item()
                )

                predicted_clause_type = (
                    self.clause_types[prediction]
                    if prediction < len(self.clause_types)
                    else "unknown"
                )
    ...
    def compute_similarity_matrix(self, save_matrix: bool = True):
        ...
        self.similarity_matrix = np.dot(
            self.normalized_embeddings, self.normalized_embeddings.T
        )
    ...
    def find_similar_clauses(
        ...
        use_legalbert_classification: bool = True,
    ) -> List[Dict]:
        """
        ...
        if use_legalbert_classification and self.binary_model:
            has_answer, predicted_type, confidence = self.classify_query_with_legalbert(
                query_text
            )
            ...
            if not has_answer:
                logger.warning(
                    f"LegalBERT classified query as not relevant (confidence: {confidence:.3f})"
                )
        ...
        if clause_type_filter:
            ...
            if type_key in self.embedding_index:
                type_indices = self.embedding_index[type_key].get(
                    clause_type_filter, []
                )
            else:
                # Fallback to ground truth types
                type_indices = self.embedding_index.get(
                    "clause_type_to_indices", {}
                ).get(clause_type_filter, [])
        ...
        candidate_indices = [
            candidate_indices[i] for i, valid in enumerate(valid_mask) if valid
        ]
        ...
        if len(similarities) > 0:
            ...
            if query_classification:
                logger.info(
                    f"Query classified as: {query_classification['predicted_clause_type']} "
    ...
    def find_similar_clauses_by_id(
        self,
        clause_id: str,
        top_k: int = 10,
        ...
        source_clause_type = source_clause.get(
            "predicted_clause_type", source_clause.get("clause_type", "unknown")
        )
        ...
        if prefer_same_type and source_clause_type != "unknown":
            for i in candidate_indices:
                ...
                candidate_type = candidate_clause.get(
                    "predicted_clause_type",
    ...
    def get_clause_clusters(
        ...
        if clause_type_filter:
            if (
                use_predicted_types
                and "predicted_clause_type_to_indices" in self.embedding_index
            ):
                type_indices = self.embedding_index[
                    "predicted_clause_type_to_indices"
                ].get(clause_type_filter, [])
            else:
                type_indices = self.embedding_index.get(
                    "clause_type_to_indices", {}
        ...
        if len(embeddings) < n_clusters:
            logger.warning(
                f"Not enough clauses ({len(embeddings)}) for {n_clusters} clusters"
            )
        ...
        for cluster_id, clauses in clusters.items():
            ...
            if use_predicted_types and "predicted_clause_type" in clauses[0]:
                clause_types = [
                    clause.get("predicted_clause_type", "unknown") for clause in clauses
                ]
            else:
                clause_types = [
                    clause.get("clause_type", "unknown") for clause in clauses
                ]
            ...
            cluster_stats[cluster_id] = {
                ...
                "diversity": (
                    len(set(clause_types)) / len(clause_types) if clause_types else 0
                ),
    ...
    def analyze_legalbert_performance(self) -> Dict:
        ...
        return {
            ...
            "average_type_accuracy": (
                np.mean(list(per_type_accuracy.values())) if per_type_accuracy else 0
            ),
        }

    def export_similarity_results(
        self, query_text: str, results: List[Dict], filename: str = None
    ) -> str:
    ...
    def get_engine_stats(self) -> Dict:
        ...
        stats = {
            "extraction_method": self.clause_embeddings.get(
            ...
            "classification_pipeline": self.clause_embeddings.get(
                "pipeline_version", "1.0"
            ),
            "binary_threshold": self.clause_embeddings.get("binary_threshold", "N/A"),
            "multiclass_accuracy": self.clause_embeddings.get(
                "multiclass_accuracy", "N/A"
            ),
        ...
        if "predicted_clause_type_to_indices" in self.embedding_index:
            stats["predicted_clause_types"] = len(
                self.embedding_index["predicted_clause_type_to_indices"]
            )
            stats["clause_type_distribution"] = {
                clause_type: len(indices)
                for clause_type, indices in self.embedding_index[
                    "predicted_clause_type_to_indices"
                ].items()
            }

        if "ground_truth_clause_type_to_indices" in self.embedding_index:
            stats["ground_truth_clause_types"] = len(
                self.embedding_index["ground_truth_clause_type_to_indices"]
            )
...
def main():
    ...
    results = engine.find_similar_clauses(
        query_text, top_k=5, use_legalbert_classification=True
    )
    ...
    for result in results:
        predicted_type = result.get(
            "predicted_clause_type", result.get("clause_type", "unknown")
        )
        print(
            f"  {result['rank']}. [{predicted_type}] "
        ...
        if "query_classification" in result:
            ...
            print(
                f"     Query classified as: {qc['predicted_clause_type']} "
    ...
    if clause_ids:
        ...
        similar_clauses = engine.find_similar_clauses_by_id(
            sample_clause_id, top_k=3, prefer_same_type=True
        )
        ...
        for result in similar_clauses:
            ...
            print(
                f"  {result['rank']}. [{predicted_type}] {match_indicator} "
                f"Similarity: {result['similarity_score']:.3f}"
            )
            print(f"     Answer: {result['answer_text'][:100]}...")
            print()
    ...
    if "predicted_clause_type_to_indices" in engine.embedding_index:
        clause_types = list(
            engine.embedding_index["predicted_clause_type_to_indices"].keys()
        )
        if clause_types:
            sample_type = clause_types[0]
            cluster_results = engine.get_clause_clusters(
                n_clusters=3, clause_type_filter=sample_type, use_predicted_types=True
            )
